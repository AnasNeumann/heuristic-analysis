								...
								for (int y = 0; y < m.nbrJobs; y++) {
									if (y != j && y != i) {
										...
										for (int v = 0; v < nbrOpY; v++) {
											c15.addTerm(m.I, m.varPrecedence[j][q][y][v]);
											c15.addTerm(m.I, m.varPrecedence[y][v][i][k]);
											c15.addTerm(-m.I, m.varPrecedence[j][q][i][k]);
										}
									}
								}
											...
											for (int v = 0; v < m.nbrJobs; v++) {
												if (v != y && v != i && y != j) {
													...
													for (int t = 0; t < nbrOpV; t++) {
														c12.addTerm(m.I, m.varPrecedence[i][q][v][t]);
														c12.addTerm(m.I, m.varPrecedence[v][t][y][k]);
														c12.addTerm(-m.I, m.varPrecedence[i][q][y][k]);
													}
												}
											}
